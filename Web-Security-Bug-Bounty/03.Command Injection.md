...
Now we are going to upload PHP reverse shell in the target website. Using that we can get access to that computer.
...
We can just use ; commands to execute, we don't need to use www.google.com every time.
...
nc -lvp 4444
```
Use the port you configured in the reverse shell.In my case I put 4444.
...
```
Listen in Kali terminal using netcat:
```
nc -lvp 4444
```
